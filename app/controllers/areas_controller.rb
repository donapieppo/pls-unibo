class AreasController < ApplicationController
  ...
  before_action :set_area_and_check_permission, only: %i[show edit update add_contact remove_contact]

  def show
    # si vedono le edizioni di quest'anno
    @edition_evidences = Edition.visible(current_user&.staff?).in_evidence.in_area(@area)
    @editions = Edition.visible(current_user&.staff?).this_academic_year.in_area(@area)
    @common_projects = Project.where(global: true).include_all.visible(current_user&.staff?).order(:name).all
